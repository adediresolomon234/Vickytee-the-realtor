...
import { getAdminProperties, getLeads, type Lead, type PropertyRecord } from "../../lib/storage";
...
export default async function AdminPage() {
  ...
  try { [leads, properties] = await Promise.all([getLeads(), getAdminProperties()]); } catch { /* Storage may be initializing in a local preview. */ }
  return <main className="admin-shell">
    ...
    <header className="admin-header"><div><p className="eyebrow">Private workspace</p><h1>Welcome, Victoria.</h1></div><p>Add new properties with photos and videos, control what is live, publish social updates, and review client inquiries.</p></header>
    <section className="admin-panel property-admin-panel"><p className="eyebrow">Property manager</p><h2>Add a new house</h2><p>Enter the property details, select multiple photos or videos, and publish the home directly to the public gallery.</p><PropertyManager properties={properties} /></section>
